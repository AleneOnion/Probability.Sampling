...
fully.support <- cat_analysis(
  ...
  xcoord = "xcoord",
  ycoord = "ycoord")
table <- fully.support %>% select(Category,nResp,Estimate.U,LCB95Pct.U,UCB95Pct.U)
hux <- as_hux(table)
number_format(hux) <- 2
theme_plain(hux)
...
results <- results %>% 
  ...
  filter(Category==0) %>% 
  mutate(Category="Non-Excursions") %>% 
  select(Indicator,Category,pct,pct_lcb,pct_ucb)

results$Indicator <- factor(results$Indicator, levels=c("ammonia","dissolved_oxygen","nitrite","ph","fully supporting"))

#side by side plots
print(ggplot(results,aes(x=Indicator,y=pct)) +
  ...
  labs(y="Percent of total",x="Fully supporting fishing use"))
