...
att<-att %>% 
  ...
  mutate(chloride=case_when(
    `CHLORIDE_OW_TOTAL` <= 25 ~ "Low",
    between(`CHLORIDE_OW_TOTAL`,25,100) ~ "Medium",
    `CHLORIDE_OW_TOTAL` >= 100 ~ "High",
...
totals<-chloride %>% 
  select(Indicator,Category,Estimate.U) %>% 
  rename(total=Estimate.U)
totals$total <- chloride$Estimate.U[4]
...
table$Category <- factor(table$Category, levels=c("Low", "Medium", "High"))
