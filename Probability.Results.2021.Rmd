...
#hypolimnetic means for ph and do
hypo<-lab %>% 
...
truncate<-hypo %>% 
  select(LAKE_ID,SAMPLE_DATE) %>% 
  distinct()
thermocline<-merge(truncate,thermocline,by=c('LAKE_ID','SAMPLE_DATE'),all.x=TRUE)
...
att<-merge(lab,hypo,by=c("LAKE_ID","SAMPLE_DATE","chemical_name"),all=TRUE) %>% 
  filter(chemical_name %in% c('CHLOROPHYLL A_OW_TOTAL',
                              ...
                              "PH_epi",
                              "PH_hypo",
                              "DISSOLVED OXYGEN_hypo")) %>% 
  mutate(result_value=ifelse(is.na(result_value.x),result_value.y,result_value.x)) %>% 
  select(-result_value.x,-result_value.y,-profile_depth)
att<-merge(att,epi,by=c("LAKE_ID","SAMPLE_DATE","chemical_name"),all=TRUE) %>% 
  filter(chemical_name %in% c('CHLOROPHYLL A_OW_TOTAL',
                              'PHOSPHORUS, TOTAL_OW_TOTAL',
                              "MICROCYSTIN_OW_NA",
                              "NITROGEN, NITRATE-NITRITE_OW_TOTAL",
...
att<-att %>% 
  ...
  mutate(d.oxygen=case_when(
    `DISSOLVED OXYGEN_hypo`<=3 ~ "Poor",
    between(`DISSOLVED OXYGEN_hypo`, 3, 5) ~ "Fair",
    `DISSOLVED OXYGEN_hypo`>=5 ~ "Good")) %>% 
  #nutrient limitation
...
ph_hypo<-ph %>% 
  ...
  filter(Category!="Total") %>% 
  gather(percent,results,-Indicator,-Category) %>% 
  mutate(Indicator=ifelse(Indicator=="ph_hypo","pH condition category",Indicator))
...
table$Category <- factor(table$Category, levels=c("Acidic","Circumneutral","Alkaline"))
...
ggplot(table,aes(x=Indicator,y=Percentage,fill=Category)) +
  geom_bar(stat = "identity",position="stack")+
  labs(title="Distribution of trophic classes across NYS Ponded Waters",y="Percent of Total")
