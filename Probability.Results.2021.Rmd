...
att<-merge(lab,epi,by=c("LAKE_ID","SAMPLE_DATE","chemical_name"),all=TRUE) %>% 
  filter(chemical_name %in% c('CHLOROPHYLL A_OW_TOTAL',
                              'PHOSPHORUS, TOTAL_OW_TOTAL',
                              "MICROCYSTIN_OW_NA",
                              "NITROGEN, NITRATE-NITRITE_OW_TOTAL",
                              "NITROGEN, KJELDAHL, TOTAL_OW_TOTAL",
                              ...
                              "NITROGEN, NITRATE (AS N)_OW_TOTAL",
                              "TRUE COLOR_OW_TOTAL",
                              "SPECIFIC CONDUCTANCE_epi",
                              "PH_epi",
                              "DISSOLVED OXYGEN_epi",
                              "DEPTH, SECCHI DISK DEPTH_SD_NA",
                              "CALCIUM_OW_TOTAL",
                              "CHLORIDE_OW_TOTAL")) %>% 
...
att<-att %>% 
  ...
  mutate(conductance=case_when(
    `SPECIFIC CONDUCTANCE_epi`<=125 ~ "Softwater",
    between(`SPECIFIC CONDUCTANCE_epi`, 125, 250) ~ "Average",
    `SPECIFIC CONDUCTANCE_epi`>=250 ~ "Hardwater")) %>%
  ...
  mutate(ph=case_when(
    `PH_epi`<=6.5 ~ "Acidic",
    between(`PH_epi`, 6.5, 7.5) ~ "Circumneutral",
    between(`PH_epi`, 7.5, 8.5) ~ "Slightly alkaline",
    `PH_epi`>=8.5 ~ "Highly alkaline")) %>% 
  #leech
  ...
  mutate(chloride=case_when(
    `CHLORIDE_OW_TOTAL` <= 100 ~ "Low (<100 mg/L)",
    between(`CHLORIDE_OW_TOTAL`,100,250) ~ "Medium (100-250 mg/L)",
    `CHLORIDE_OW_TOTAL` >= 250 ~ "High (>250 mg/L)",
    Eval_Status=="Target_Sampled" & is.na(`CHLORIDE_OW_TOTAL`) ~ "Low (<100 mg/L)"))
...
CatExtent2 <- cat_analysis(
  dframe=att,
  vars=c("chla_trophic","phos_trophic","secchi"),
  subpops = ,
  siteID = "siteID",
  weight = "WgtAdj",
  xcoord = "xcoord",
...
totals[totals$Indicator=="chla_trophic",3] <- CatExtent2$Estimate.U[4]
totals[totals$Indicator=="phos_trophic",3] <- CatExtent2$Estimate.U[8]
totals[totals$Indicator=="secchi",3] <- CatExtent2$Estimate.U[12]
...
forplot2<-CatExtent2 %>% 
  ...
  mutate(Indicator=ifelse(Indicator=="chla_trophic","Trophic Status from ChlA",Indicator),
         ...
         Indicator=ifelse(Indicator=="secchi","Trophic Status from Secchi",Indicator))
...
ggplot(table,aes(x=Indicator,y=Percentage,fill=Category)) +
...
totals<-nlimit %>% 
  select(Indicator,Category,Estimate.U) %>% 
  rename(total=Estimate.U)
totals$total <- nlimit$Estimate.U[4]
nlimit<-merge(nlimit,totals,by=c('Indicator','Category'),all.x = TRUE)
...
forplot<-nlimit %>% 
  ...
  filter(Category!="Total") %>% 
  gather(percent,results,-Indicator,-Category) %>% 
  mutate(Indicator=ifelse(Indicator=="N_LIMIT","Nutrient limitation category",Indicator))
...
totals<-chloride %>% 
  select(Indicator,Category,Estimate.U) %>% 
  rename(total=Estimate.U)
totals$total <- chloride$Estimate.U[3]
chloride<-merge(chloride,totals,by=c('Indicator','Category'),all.x = TRUE)
...
forplot<-chloride %>% 
  ...
  filter(Category!="Total") %>% 
  gather(percent,results,-Indicator,-Category) %>% 
  mutate(Indicator=ifelse(Indicator=="chloride","Chloride",Indicator))
...
ggplot(table,aes(x=Category,y=Percentage)) +
  ...
  facet_wrap(~Indicator,scales = "free_y")+
  ylim(0,100)+
  labs(title="Chloride conditions across NYS Ponded Waters",y="Percent of Total",x="Condition category")
...
totals<-zebra %>% 
  select(Indicator,Category,Estimate.U) %>% 
  rename(total=Estimate.U)
totals$total <- zebra$Estimate.U[4]
zebra<-merge(zebra,totals,by=c('Indicator','Category'),all.x = TRUE)
...
forplt10<-zebra %>% 
  ...
  filter(Category!="Total") %>% 
  gather(percent,results,-Indicator,-Category) %>% 
  mutate(Indicator=ifelse(Indicator=="zebra","Susceptibility",Indicator))

table <- forplt10 %>% 
  spread(percent,results)  %>% 
  rename(Percentage=pct,Lower95Confidence=pct_lcb,Upper95Confidence=pct_ucb)

table$Category <- factor(table$Category, levels=c("Not susceptible", "May be susceptible", "Highly susceptible"))
...
ggplot(table,aes(x=Category,y=Percentage)) +
  ...
  facet_wrap(~Indicator,scales = "free_y")+
  ylim(0,100)+
  labs(title="Distribution of zebra mussel susceptibility across NYS Ponded Waters",y="Percent of Total",x="Condition category")
...
totals<-CatExtent %>% 
  select(Indicator,Category,Estimate.U) %>% 
  rename(total=Estimate.U)
totals[totals$Indicator=="CHLA_threshold",3] <- CatExtent$Estimate.U[8]
totals[totals$Indicator=="TN_threshold",3] <- CatExtent$Estimate.U[12]
totals[totals$Indicator=="TP_threshold",3] <- CatExtent$Estimate.U[4]
...
frame<-att %>% 
  ...
  mutate(Evaluated_Lakes = case_when(
...
frame$study <- factor(frame$study, levels=c("LMAS_Sites","Probability_Sites","Target_Sampled","Evaluated_Lakes","Design_Frame"))
...
print(
  ggplot(att,aes(x=`PHOSPHORUS, TOTAL_OW_TOTAL`,y=`TRUE COLOR_OW_TOTAL`,color=leech))+
    geom_point()+
    geom_vline(xintercept=0.030)+
...
totals<-CatExtent2 %>% 
  select(Indicator,Category,Estimate.U) %>% 
  rename(total=Estimate.U)
totals$total <- CatExtent2$Estimate.U[5]
CatExtent2<-merge(CatExtent2,totals,by=c('Indicator','Category'),all.x = TRUE)
...
ggplot(table,aes(x=Category,y=Percentage)) +
  ...
  facet_wrap(~Indicator,scales = "free_y")+
  ylim(0,100)+
  labs(title="Lake nutrient-color status across NYS Ponded Waters",y="Percent of Total",x="Lake nutrient-color status")
