...
author: "Sabrina Xie and Alene Onion and Tony Olsen of USEPA"
...
lab<-newdata %>%
  ...
  filter(!is.na(RSLT_RESULT_VALUE),
         ...
         combined %in% c('CHLOROPHYLL A_OW_TOTAL','PHOSPHORUS, TOTAL_OW_TOTAL',"MICROCYSTIN_OW_NA","NITROGEN, NITRATE-NITRITE_OW_TOTAL")) %>%
...
att<-att %>% 
  ...
  mutate(TN_threshold=case_when(
    `NITROGEN, NITRATE-NITRITE_OW_TOTAL`<=0.428 ~ "Good",
    between(`NITROGEN, NITRATE-NITRITE_OW_TOTAL`, 0.428, 0.655) ~ "Fair",
    `NITROGEN, NITRATE-NITRITE_OW_TOTAL`>=0.655 ~ "Poor")) %>%
  ...
  mutate(microcystin=case_when(
    `MICROCYSTIN_OW_NA`<=0.1 ~ "Below MDL",
    between(`MICROCYSTIN_OW_NA`, 0.1, 8) ~ "Below thresholdd",
    `MICROCYSTIN_OW_NA`>=8 ~ "Above threshold")) %>%
  #dissolved oxygen
...
att<-att %>% 
  mutate(WgtAdj=NA,
         WgtAdj=ifelse(PROB_CAT=="(1,4]",((1828/29)),WgtAdj),
         WgtAdj=ifelse(PROB_CAT=="(4,10]",((2490/15)),WgtAdj),
         WgtAdj=ifelse(PROB_CAT=="(10,20]",((1003/18)),WgtAdj),
         WgtAdj=ifelse(PROB_CAT=="(20,50]",((616/21)),WgtAdj),
         WgtAdj=ifelse(PROB_CAT==">50",((500/33)),WgtAdj))
