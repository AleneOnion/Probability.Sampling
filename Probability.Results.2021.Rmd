...
totals<-secchi %>% 
  select(Indicator,Category,Estimate.U) %>% 
  rename(total=Estimate.U)
totals$total <- secchi$Estimate.U[4]
secchi<-merge(secchi,totals,by=c('Indicator','Category'),all.x = TRUE)
...
forplt10<-secchi %>% 
  ...
  filter(Category!="Total") %>% 
  gather(percent,results,-Indicator,-Category) %>% 
  mutate(Indicator=ifelse(Indicator=="secchi","Nutrient category",Indicator))

table <- forplt10 %>% 
  spread(percent,results)  %>% 
  rename(Percentage=pct,Lower95Confidence=pct_lcb,Upper95Confidence=pct_ucb)

table$Category <- factor(table$Category, levels=c("Oligotrophic", "Mesotrophic", "Eutrophic"))
...
ggplot(table,aes(x=Category,y=Percentage)) +
  ...
  facet_wrap(~Indicator,scales = "free_y")+
  ylim(0,100)+
  labs(title="Distribution of Secchi disk depths across NYS Ponded Waters",y="Percent of Total",x="Condition category")
