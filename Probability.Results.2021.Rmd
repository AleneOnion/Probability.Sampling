...
forplot3$Category <- factor(forplot3$Category, levels=c("Good","Fair","Poor"))
...
ggplot(forplot4, aes(x=Category,fill=study)) +
...
n_att<-lab %>% spread(chemical_name,result_value,fill = NA) 
n_att<-n_att %>% 
  # create total nitrogen
  mutate(`NITROGEN, TOTAL`=case_when(
    !is.na(`NITROGEN, KJELDAHL, TOTAL_OW_TOTAL`) ~
...
forplot5$Category <- factor(forplot5$Category, levels=c("Good","Fair","Poor"))
...
ggplot(forplot5, aes(x=Category,fill=study)) +
  ...
  geom_errorbar(aes(ymin=pct_lcb, ymax=pct_ucb), width=.2,
                 position=position_dodge(.9))+
  facet_wrap(~Indicator,scales = "free_y")+
  labs(title="EPA condition categories in the Probability Study versus LMAS samples since 2010",y="Percent of Total",x="EPA Condition Classes")
```
