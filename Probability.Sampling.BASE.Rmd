...
rm(list=setdiff(ls(), c('newdata',"att","framesize_LC","sites")))
...
ggplot(table[table$Indicator %in% c("phos_trophic","chla_trophic","secchi"),],aes(y=Estimate.P,x=Category,fill=Category))+
  geom_col()+
  ylim(0,100)+
  geom_errorbar(aes(ymin=LCB95Pct.P,ymax=UCB95Pct.P),width=0.2)+
  coord_flip()+
  ...
  scale_fill_discrete(name = "Trophic status", labels = c("Oligotrophic", "Mesotrophic", "Eutrophic"))
