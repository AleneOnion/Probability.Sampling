...
myvars <- c("CHLOROPHYLL_ug_L","NITROGEN_mg_L","PHOSPHORUS_mg_L","CHLORIDE_mg_L","DISSOLVED_OXYGEN_mg_L","DOC_mg_L","CALCIUM_mg_L","SECCHI_m","TRUE_COLOR","SPEC_CONDUCTANCE","AMMONIA_N","MAGNESIUM_OW_TOTAL")
...
ggplot(analysis$CDF,aes(x=Value,y=Estimate.P,ymin=LCB95Pct.P,ymax=UCB95Pct.P))+
  geom_line()+
  geom_point()+
...
myvars <- c("CHLOROPHYLL_ug_L","NITROGEN_mg_L","PHOSPHORUS_ug_L","CHLORIDE_mg_L","DISSOLVED_OXYGEN_mg_L","DOC_mg_L","CALCIUM_mg_L","TRUE_COLOR","SPEC_CONDUCTANCE","AMMONIA_N","MAGNESIUM_OW_TOTAL")
...
all <- rbind(NY,NH,NAP,NLA) %>%
  mutate(Indicator=case_when(
    ...
    Indicator=="CALCIUM_mg_L"~"Calcium (mg/L)*",
    Indicator=="TRUE_COLOR"~"Color (PCU)*",
    Indicator=="SPEC_CONDUCTANCE"~"Conductance (mS/cm)*",
    Indicator=="MAGNESIUM_OW_TOTAL"~"Magnesium (ug/L)*")) %>% 
  ...
  filter((Indicator=="Chlorophyll-a (ug/L)"&Value<50)|
           ...
           (Indicator=="Magnesium (ug/L)*"&Value<25000))

table <- data.frame(Indicator=c("Chlorophyll-a (ug/L)","Dissolved oxygen (mg/L)*","Total nitrogen (mg/L)","Total phosphorus (ug/L)"),
...
plot <- ggplot(all,aes(x=Value,y=Estimate.P,color=Study,shape=Study,linetype=Study))+
  ...
  facet_wrap(.~Indicator, scales="free")+
  labs(title="Cumulative distribution functions", subtitle="Asterisk (*) denotes parameters for which New Hampshire is not included, extreme values are truncated",y="Percent of total")
...
frame$study <- factor(frame$study, levels=c("Statewide","LMAS"))


plot <- ggplot(frame, aes(x=study,y=percentage,fill=study)) +
...
plot1 <- set_panel_size(p = plot, width = unit(4, "cm"), height = unit(4,"cm"))
...
```
```{r combined plot}
plot2 <- ggplot(forplot[forplot$Indicator=="Phosphorus",], aes(x=Category,fill=study)) +
  geom_bar_pattern(stat = "identity", position = position_dodge(),
                   aes(y=Estimate.P,
                       pattern_type = study,
                       pattern_fill = study), 
