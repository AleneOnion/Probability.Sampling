...
chem <- chem %>% 
  filter(ANALYTE %in% c("NTL","CHLA","PTL","NITRATE_N"),
         NARS_FLAG == "") %>% 
...
totals<-nlimit %>% 
  select(Indicator,Category,Subpopulation,Estimate.U) %>% 
  rename(total=Estimate.U)
totals$total <- nlimit$Estimate.U[8]
nlimit<-merge(nlimit,totals,by=c('Indicator','Category','Subpopulation'),all.x = TRUE)

forplot<-nlimit %>% 
  mutate(pct=(Estimate.U/total)*100,
         pct_lcb=(LCB95Pct.U/total)*100,
         pct_ucb=(UCB95Pct.U/total)*100) %>% 
  select(Indicator,Category,Subpopulation,pct,pct_lcb,pct_ucb) %>% 
  distinct() %>% 
  filter(Category!="Total",
         Subpopulation=="yes") %>% 
  gather(percent,results,-Indicator,-Category,-Subpopulation) %>% 
...
ggplot(table,aes(x=Category,y=Percentage)) +
  ...
  facet_wrap(~Indicator,scales = "free_y")+
  ylim(0,100)+
  labs(title="Distribution of nutrient limitation across NLA Ponded Waters",y="Percent of Total",x="Condition category")

forplotsss<-forplot %>% 
  spread(percent,results) %>% 
  rename(percent=pct) %>% 
  mutate(study="NLA") %>% 
  add_row(Indicator="Nutrient limitation category", #ny prob
          Category="Co-limited",
          study="NYS",
          percent=12.96,
          pct_lcb=1.15,
  ...
  add_row(Indicator="Nutrient limitation category", #ny prob
          ...
          pct_lcb=6.83,
          pct_ucb=21.53) %>%
  add_row(Indicator="Nutrient limitation category", #nap
          Category="Co-limited",
          study="NAP",
          percent=4.56,
          pct_lcb=0.00,
...
ggplot(forplotsss, aes(x=Category)) +
  geom_point(aes(y=percent,color=study),stat = "identity", position=position_dodge(width=0.2))+
  geom_errorbar(aes(ymin=pct_lcb, ymax=pct_ucb,color=study), width=.25, position=position_dodge(width=0.2))+
  facet_wrap(~Indicator,scales = "free_y")+
  ylim(0,100)+
  labs(title="Nutrient limitation in NLA vs. NYS",y="Percent of Total",x="Limiting nutrient")
...
ggplot(forplot6, aes(x=Category,fill=study)) +
  ...
  facet_wrap(~Indicator,scales = "free_y")+
  ylim(0,100)+
  labs(title="EPA condition categories in  NLA vs. Northern Appalachian vs. NY",y="Percent of Total",x="EPA Condition Classes")
```
